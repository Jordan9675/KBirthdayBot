...
def locate_birthday_table(soup: BeautifulSoup) -> BeautifulSoup:
    return soup.select_one("#table_1")


def locate_rows_from_birthday_table(table: BeautifulSoup) -> BeautifulSoup:
...
def get_birthdays() -> list:
    ...
    table = locate_birthday_table(soup)
    rows = locate_rows_from_birthday_table(table)
